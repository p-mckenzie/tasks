...
Group.create(title: 'Smith family chores',
             description: "Space to keep track of recurring household tasks, like laundry & doctor's appointments")

User.create(first_name: 'Jane', last_name: 'Smith', email: 'js@test.com', password: 'testtest',
            password_confirmation: 'testtest')
UserGroupAssignment.create(user: User.last, group: Group.first, admin: true)

User.create(first_name: 'Abigail', last_name: 'Smith', email: 'as@test.com', password: 'testtest',
            password_confirmation: 'testtest')
...
Group.create(title: 'Petfinder dashboard project',
             description: 'Open-source community effort to optimize pet adoptions in Example county')
UserGroupAssignment.create(user: User.first, group: Group.last)

User.create(first_name: 'Taylor', last_name: 'Sanchez', email: 'ts@test.com', password: 'testtest',
            password_confirmation: 'testtest')
...
Task.select(&:recurring?).each do |task|
  loop do
    instance = task.current_instance
    break if instance.due_date > Date.today
